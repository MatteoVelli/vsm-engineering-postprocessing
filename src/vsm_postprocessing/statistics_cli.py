...
from .statistics_engine import calculate_statistics, export_statistics


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="vsm-stats",
        description="Calculate deterministic configured statistics for raw and math VSM channels.",
    )
    parser.add_argument("input_file", help="Path to the source .xlsx or .csv file")
    parser.add_argument("--config", required=True, help="Path to the versioned YAML statistics file")
    parser.add_argument("--math-config", help="Optional math-channel YAML file used before statistics")
    parser.add_argument("--output-dir", default="outputs/statistics", help="Output directory")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        outputs = export_statistics(result, args.output_dir)
    except VSMPostProcessingError as exc:
        print(f"ERROR: {exc}", file=sys.stderr)
        return 2

    print("VSM statistics completed")
    print("  status: PASS")
    print(f"  samples: {result.sample_count}")
    print(f"  available channels: {len(result.channels_by_id)}")
    print(f"  statistics calculated: {result.statistic_count}")
    print(f"  reference comparisons: {result.comparison_count}")
    print("  results:")
    for index, item in enumerate(result.statistics, start=1):
        ...
        print(
            f"    {index:02d}. {item.statistic_id} | {item.operation} | "
            f"{item.value:.12g} [{item.channel_unit or '-'}]{comparison}"
        )
    print("  outputs:")
